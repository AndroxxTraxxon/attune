...
- **GitHub Actions binary bundle publishing** (`.github/workflows/publish.yml`): Rust binary bundles are exchanged between publish jobs with `actions/upload-artifact` / `actions/download-artifact`, avoiding dependency on Gitea generic packages. The publish workflow also creates a combined `attune-build-artifacts-<image_tag>.zip` artifact containing all workflow-produced archives/packages plus `metadata.json`; tag builds attach that zip to the GitHub Release.
