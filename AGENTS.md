...
- **Key Settings**:
  ...
  - `worker.execution_log_retention_policy` / `worker.execution_log_retention_limit` - Retention settings for per-execution action stdout/stderr log artifacts. Defaults to `days` / `7`; policy supports `versions`, `days`, `hours`, or `minutes`. Individual action and sensor rows may override log retention via nullable `log_retention_policy` / `log_retention_limit` fields. Sensor log artifacts have an independent default of `versions` / `4`.
