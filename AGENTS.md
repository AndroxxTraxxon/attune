...
- **Pack**: Bundle of automation components (actions, sensors, rules, triggers, runtimes). API-created non-standard packs are scoped to the creating identity via `pack.installed_by`; system/standard packs remain globally visible. Cross-owner pack list/detail/update/delete access requires a meaningful constrained `packs:*` grant (for example owner, pack ref, ref, or id constraints) rather than a broad unconstrained admin grant.
...
- **Execution-local agent MCP pattern**: AI/agent actions running inside a worker should prefer the execution-scoped `ATTUNE_API_TOKEN` already injected by the worker and spawn the local `/opt/attune/agent/attune-mcp` binary over stdio. The core pack reference action `core.run_agent_command` exports `ATTUNE_MCP_COMMAND`, `ATTUNE_MCP_TRANSPORT=stdio`, and `ATTUNE_AGENT_STATE_DIR` for this purpose.
...
- **Standard Environment Variables**: Worker provides execution context via `ATTUNE_*` environment variables:
  ...
  - `ATTUNE_API_TOKEN` - Execution-scoped API token (always present). Carries the **triggering identity's `sub` claim** (the user who invoked the action via the API, the system identity for rule-driven enforcement, or the `requested_by_identity` of the leased queue items for queue dispatches). Workflow children and retries inherit the parent execution's executor. The token is subject to the same identity-based RBAC as a regular access token (driven by `execution.executor`), so an action triggered by a low-privilege user can only call back into the API with that user's permissions — not with system-level privileges. Routes that explicitly gate on `token_type == Access` still bypass RBAC for execution tokens for backward compatibility with the Slack pack inquiry-response flow and the in-action MCP server.
