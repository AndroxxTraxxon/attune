...
- **Gitea Actions binary bundle publishing** (`.gitea/workflows/publish.yml`): ORAS is pinned to `1.2.3` for binary bundle push/pull because ORAS 1.3.x requests wildcard registry token scopes that Gitea/Forgejo registries can reject with 401 even when repository pull/push credentials are valid. Do not unpin without validating artifact push/pull against the target registry.
