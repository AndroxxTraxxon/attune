...
- **System Alert**: The core pack defines `core.alert` for operational exceptions. Components use `attune_common::system_alert` to create an event and publish `EventCreated`; current emitters include unexpected non-cordoned worker loss, execution abandonment caused by worker loss, and repeated managed sensor-process failures while active rules depend on the sensor. Alert payloads include severity, category, failure type, component type/id/ref, worker role, summary, details, and correlation id.
...
**Table Count**: 39 tables total in the schema (including `runtime_version`, `artifact_version`, `integration_token`, `sensor_process`, `work_queue`, `work_queue_item`, `work_queue_dispatch`, `pack_registry_index`, 3 `*_history` hypertables, and the `event`, `enforcement`, + `execution` hypertables)
...
- **Artifact File Transport**: Abstracts HOW file content is transferred between workers/sensors and the API, enabling operation without shared Docker volumes. Defined in `crates/common/src/artifact_transport/`. Two implementations:
  - **`VolumeTransport`** (`volume.rs`): Direct filesystem I/O (shared volume, fast path). Used when the API and worker share a mounted `artifacts_dir`.
  ...
  - **Worker integration**: `ActionExecutor` carries `Arc<dyn ArtifactFileTransport>`. File operations (artifact finalization, stderr log promotion, directory creation) use the transport instead of direct `tokio::fs` calls. Pending stderr files remain local; promoted files go through transport.
...
- **Sensor Per-Process Rotating Logs**: Each sensor gets dedicated stdout/stderr log files at `{artifacts_dir}/sensors/{sensor_ref}/stdout.log` and `stderr.log`. Managed by `RotatingLogWriter` (`crates/sensor/src/sensor_log.rs`) with lazy file open, size-based rotation (default 10MB per file, 5 rotated files). Dual output: lines are written to both the rotating file AND forwarded to tracing. Sensor log artifacts are auto-registered in DB with refs `sensor.{sensor_ref}.stdout` / `sensor.{sensor_ref}.stderr` (scope: Sensor, type: FileText, 90-day retention). API endpoints: `GET /api/v1/sensors/{sensor_ref}/logs` (list streams) and `GET /api/v1/sensors/{sensor_ref}/logs/{stream}` (download content).
